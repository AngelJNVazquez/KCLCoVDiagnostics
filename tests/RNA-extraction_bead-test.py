...
def run(protocol: protocol_api.ProtocolContext):
    ...
    incubatonWater = 5 # After mixing. No magnet.
    ...
    def well_mix(loc, vol, reps, height=generalHeight, moveSide=0):
        """Aspirates <vol> from bottom of well and dispenses it from <height> <reps> times"""
        loc1 = loc.bottom().move(types.Point(x=0+moveSide, y=0, z=0.6))
    ...
    def slow_transfer(vol, src, to, extra=0):
        ...
        while tvol > tipVolume:
            ...
            p300.transfer(tipVolume, src.bottom().move(types.Point(x=0, y=0, z=0.5)),
            to.top(topOffset), new_tip="never", air_gap=extra)
            protocol.delay(seconds=2)
            p300.blow_out()
            tvol -= tipVolume
        p300.dispense(20, src.top() )
        p300.transfer(tvol,src.bottom().move(types.Point(x=0, y=0, z=0.5)),
        to.top(topOffset), new_tip="never", air_gap=extra)
        protocol.delay(seconds=2)
        p300.dispense(20)
    ...
    def adding_step(vol, reagent, reagentName, incubationTime, columnID,
    mixVol=washMixing, repeats=mixRepeats, mixReagent=False, magnetTime=True, moveSide=0,
    extraVol=0, incub=True):
        ...
        for index, ID in enumerate(columnID):
            ...
            if (mixReagent==True):
                ...
                well_mix(loc=beads, vol=beadsMixing, reps=beadsMixRepeats, height=beadsHeight, moveSide=moveSide)
                p300.blow_out(beads.top())
                mixReagent=False
            if (extraVol != 0):
                slow_transfer(vol=vol, src=reagent, to=deepPlate[ID], extra=extraVol)
            else:
                slow_transfer(vol=vol, src=reagent, to=deepPlate[ID])
            well_mix(loc=deepPlate[ID], vol=mixVol, reps=repeats, moveSide=moveSide)
            remove_tip(currentip)
        #Incubation
        if magnetTime==True:
    ...
    columnID=columnID, magnetTime=False, incub=False)
    #INCUBATION 1: NO INCUBATION
    ...
    columnID=columnID, mixReagent=True, magnetTime=False, extraVol=10, incub=False)
    #INCUBATION 2: 5 NO INCUBATION WITHOUT MAGNET
    ...
    protocol.comment("REMEMBER TO CHECK AT 5 IF EVERYTHING IS DRY, MAN")
    adding_step(vol= dilutionVol, reagent=water, reagentName="RNAse-free water",
    incubationTime=10, columnID=columnID, mixVol=waterMixing,
    magnetTime=False, repeats=waterMixRepeats, moveSide=1) #Moving tip on top of pellet
        #INCUBATION 7: 5 min incubaton WITHOUT magnet [Total: 36 min]
    protocol.comment("Engaging magnet now!")
    magneto.engage(height=magnetHeight)
    protocol.delay(minutes=incubationWaterMagnet)
        #INCUBATION 7: 1 min incubaton WITH magnet [Total: 37 min]
